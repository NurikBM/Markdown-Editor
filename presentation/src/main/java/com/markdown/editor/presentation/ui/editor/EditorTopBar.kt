...
private fun EditorDesktopActions(
    ...
    IconButton(onClick = { onIntent(EditorIntent.SetDocumentLocked(!state.isDocumentLocked)) }) {
        Icon(
            imageVector = if (state.isDocumentLocked) Icons.Default.Lock else Icons.Default.LockOpen,
            contentDescription = if (state.isDocumentLocked) "Remove lock" else "Lock with biometrics",
...
private fun LockActionMenuItem(
    ...
    DropdownMenuItem(
        text = { Text(if (isLocked) "Remove Lock Protection" else "Protect with Biometrics") },
        leadingIcon = {
            Icon(
        ...
        onClick = {
            onDismiss()
            onIntent(EditorIntent.SetDocumentLocked(!isLocked))
        }
    )
}
